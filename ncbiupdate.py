...
def check_exist(accession, col_query, col_updatelog, upfile):
    query = {}
    query['ACCESSION'] = accession

    ins = col_query.find(query).sort("_id", pymongo.ASCENDING)
...
def read_update(filename):
    ...
    col_updatelog = db_mongo['updatelog']
        ...
        while current_line:
            if l_num >= 0:
                ...
                if current_line.startswith('ACCESSION'):
                    ...
                    is_exist = check_exist(query['ACCESSION'], col_query, col_updatelog, filename)
...
#                    new_id = r_num
                    ...
                    if bson_meta.__sizeof__() < 16777216:
                        ...
                        if bson_seq.__sizeof__() < 16777216:
                            ...
                            if not is_exist:    
                                col_meta.remove({'ACCESSION':query['ACCESSION']})
                                col_query.remove({'ACCESSION':query['ACCESSION']})
                                col_seq.remove({'ACCESSION':query['ACCESSION']})
...
#                                col_seq.insert(seq)
#                            else:
#                                col_meta.update({'ACCESSION':query['ACCESSION']}, meta)
#                                col_query.update({'ACCESSION':query['ACCESSION']}, query)
#                                col_seq.update({'ACCESSION':query['ACCESSION']}, seq)
...
if __name__ == '__main__':
    ...
    for op, value in opts:
        if op == "-a":
            ...
            print "python ncbiupdate.py -a host:port -i srcfile"
            print "Create by dysj4099@gmail.com May.7 2013"
            sys.exit()
...
#    srcfile = '/home/jingshao/ncbi_data/daily-nc/con_nc.0420.flat.gz'
    ...
    print 'update: '+ str(r_num) + ' error: ' + str(e_num) + ', cost: ' + str((end_ms - start_ms).seconds) + ' s'
    pass
